...
/// No-DC mirror of [`run_server`]: serves the parent certificate directly,
/// no delegated credential.
async fn run_server_no_dc(
    listener: TcpListener,
    cert: Certificate,
) -> Result<String, String> {
    let (socket, _) = listener
...
/// No-DC mirror of [`run_client`]: verifies the server's cert against the
/// parent CA path without advertising or checking a delegated credential.
async fn run_client_no_dc(
    addr: std::net::SocketAddr,
    ca: std::path::PathBuf,
) -> Result<(), String> {
    let stream = tokio::net::TcpStream::connect(addr)
    ...
    let client = ClientTlsContext::new_no_dc(ca.to_str().ok_or("ca path utf-8")?)
        .map_err(|e| format!("client ctx (no-dc): {e}"))?;
...
/// Same EOF regression but exercising the non-DC (plain TLS) path so the
/// shutdown/EOF fix doesn't quietly regress on that side either.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn server_observes_eof_after_client_shutdown_no_dc() {
    let (cert, cert_path) =
    ...
    let server_handle = tokio::spawn(async move {
        let _ = started_tx.send(());
        run_server_no_dc(listener, cert).await
    });

    started_rx.await.expect("server started");

    let client_handle = tokio::spawn(run_client_no_dc(addr, cert_path));
